...
class PredictionService:
    ...
    @staticmethod
    def prophet_predict(
        data: pd.DataFrame, forecast_years: int, interval_width: float
    ...
    @staticmethod
    def arima_predict(
        data: pd.DataFrame, forecast_years: int, confidence: float
    ) -> Tuple[List[PredictionPoint], Dict, Dict[str, float]]:
        ...
        forecast = results.get_forecast(
            steps=forecast_years * 365, alpha=1 - confidence
        )

        # Create date range for predictions
        last_date = data.ds.iloc[-1]
        dates = pd.date_range(
            start=last_date + timedelta(days=1), periods=forecast_years * 365, freq="D"
        )

        # Convert to response format
        predictions = [
            PredictionPoint(date=date, value=mean, lower_bound=lower, upper_bound=upper)
        ...
        model_params = {"order": results.model.order, "method": results.model.method}
        ...
        for _ in range(forecast_years * 365):
            with torch.no_grad():
                ...
                last_sequence = torch.cat(
                    [last_sequence[:, 1:, :], pred.reshape(1, 1, 1)], dim=1
                )
