...
async def get_token_data(token_id: str) -> Dict:
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Error fetching token data: {str(e)}"
        )
...
async def get_trending_tokens() -> List[Dict]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Error fetching trending tokens: {str(e)}"
        )
...
async def search_tokens(query: str, limit: Optional[int] = 10) -> List[Dict]:
    """
    ...
    try:
        ...
        sorted_coins = sorted(
            coins, key=lambda x: x.get("market_cap_rank") or float("inf")
        )
        return sorted_coins[:limit]
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Error searching tokens: {str(e)}")
