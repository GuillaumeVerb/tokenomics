...
async def simulate_constant_inflation(
    ...
    try:
        ...
        simulation_data = calculate_constant_inflation(
            request.initial_supply, request.inflation_rate, request.duration_in_years
        )
        ...
        raise HTTPException(
            status_code=400, detail=f"Error in simulation calculation: {str(e)}"
        )
...
async def simulate_token_burn(request: BurnSimulationRequest) -> SimulationResponse:
    try:
        ...
        return SimulationResponse(
            simulation_data=simulation_data, total_burned=total_burned
        )
    except Exception as e:
...
async def simulate_token_vesting(
    request: VestingSimulationRequest,
) -> SimulationResponse:
    try:
        simulation_data = simulate_vesting(request)
        total_vested = (
            simulation_data[-1].circulating_supply if simulation_data else None
        )
        return SimulationResponse(
            simulation_data=simulation_data, total_vested=total_vested
        )
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def simulate_token_staking(
...
def calculate_inflation(
    ...
    if config.type == "constant":
        ...
        rate = max(
            min_rate,
            Decimal(str(config.initial_rate))
            * ((Decimal("1") - decay / Decimal("100")) ** period),
        )
...
def calculate_vesting(config: Optional[VestingConfig], period: int) -> Decimal:
...
def calculate_staking(
    ...
    target_staked = current_supply * (
        Decimal(str(config.target_rate or 0)) / Decimal("100")
    )
...
async def simulate_scenario(request: ScenarioRequest) -> ScenarioResponse:
    ...
    for period in range(1, request.duration + 1):
        ...
        # 2. Calculate burn
        burned = calculate_burn(total_supply, request.burn_config, period)
        total_supply -= burned
        total_burned += burned

        # 3. Calculate vesting
        vested = calculate_vesting(request.vesting_config, period)
        total_vested += vested
    ...
    # Calculate summary
    supply_change = (
        (total_supply - request.initial_supply) / request.initial_supply * 100
    )
...
def generate_comparison_graph(
    scenarios: List[ScenarioComparison], metrics: List[str]
) -> PlotlyGraph:
    ...
    return PlotlyGraph(data=fig.to_dict()["data"], layout=fig.to_dict()["layout"])
...
def calculate_range_summary(scenarios: List[ScenarioComparison]) -> ComparisonSummary:
    ...
    def get_range(values: List[Decimal]) -> Dict[str, Decimal]:
        ...
        return {"min": to_decimal(min(values)), "max": to_decimal(max(values))}
    ...
    return ComparisonSummary(
        ...
        supply_change_range=get_range(
            [s.summary.supply_change_percentage for s in scenarios]
        ),
    )
...
async def compare_scenarios(request: ComparisonRequest) -> ComparisonResponse:
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Error in scenario comparison: {str(e)}"
        )
