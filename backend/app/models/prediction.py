...
class PredictionRequest(BaseModel):
    ...
    historical_data: List[TimeSeriesPoint] = Field(
        ..., min_items=10, description="Historical data points (date, value)"
    )
    forecast_years: int = Field(
        5, ge=1, le=10, description="Number of years to forecast"
    )
    model_type: Literal["prophet", "arima", "lstm"] = Field(
        "prophet", description="Type of prediction model to use"
    )
    confidence_interval: float = Field(
        0.95, gt=0, lt=1, description="Confidence interval for predictions (0-1)"
    )
