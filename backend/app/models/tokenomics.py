...
class InflationConfig(BaseTokenomicsModel):
    type: Literal["constant", "dynamic", "halving"] = Field(
        default=..., description="Type of inflation"
    )
    initial_rate: Decimal = Field(
        default=..., description="Initial inflation rate (%)", ge=0, le=100
    )
    min_rate: Optional[Decimal] = Field(
        default=None,
        description="Minimum inflation rate for dynamic type (%)",
        ge=0,
        le=100,
    )
    decay_rate: Optional[Decimal] = Field(
        default=None, description="Decay rate for dynamic type (%)", ge=0, le=100
    )
    halving_period: Optional[int] = Field(
        default=None, description="Period in months for halving type", gt=0
    )

    @model_validator(mode="after")
    def validate_config(self) -> "InflationConfig":
        if self.type == "dynamic" and (
            self.min_rate is None or self.decay_rate is None
        ):
...
class VestingPeriod(BaseTokenomicsModel):
    start_period: int = Field(
        default=..., ge=0, description="Start month of vesting period"
    )
    duration: int = Field(
        default=..., gt=0, description="Duration of vesting in months"
    )
    ...
    release_type: Literal["linear"] = Field(
        default="linear", description="Type of vesting release"
    )
...
class BurnConfig(BaseTokenomicsModel):
    ...
    type: Literal["continuous"] = Field(
        default=..., description="Type of burn mechanism"
    )
    rate: Decimal = Field(
        default=..., description="Monthly burn rate (%)", ge=0, le=100
    )
...
class VestingConfig(BaseTokenomicsModel):
    ...
    periods: List[VestingPeriod] = Field(
        default=..., min_items=1, description="List of vesting periods"
    )
...
class StakingConfig(BaseTokenomicsModel):
    ...
    target_rate: Decimal = Field(
        default=..., description="Target staking participation rate (%)", ge=0, le=100
    )
    reward_rate: Decimal = Field(
        default=..., description="Annual staking reward rate (%)", ge=0, le=1000
    )
...
class ScenarioRequest(BaseTokenomicsModel):
    ...
    time_step: str = Field(
        ..., pattern="^(monthly|yearly)$", description="Time step for simulation"
    )
    duration: int = Field(..., gt=0, le=360, description="Duration of simulation")
    inflation_config: Optional[InflationConfig] = Field(
        default=None, description="Inflation configuration"
    )
    burn_config: Optional[BurnConfig] = Field(
        default=None, description="Burn configuration"
    )
    vesting_config: Optional[VestingConfig] = Field(
        default=None, description="Vesting configuration"
    )
    staking_config: Optional[StakingConfig] = Field(
        default=None, description="Staking configuration"
    )
...
class NamedScenarioRequest(ScenarioRequest):
    ...
    name: str = Field(
        ..., min_length=1, max_length=50, description="Name of the scenario"
    )


class TokenPoint(BaseTokenomicsModel):
    month: int = Field(default=..., ge=0, description="Month of simulation")
    circulating_supply: Decimal = Field(
        default=..., ge=0, description="Circulating supply"
    )
    locked_supply: Optional[Decimal] = Field(
        default=None, description="Locked supply (staking/vesting)", ge=0
    )
    burned_supply: Optional[Decimal] = Field(
        default=None, description="Cumulative burned supply", ge=0
    )
    staked_supply: Optional[Decimal] = Field(
        default=None, description="Currently staked supply", ge=0
    )
    rewards_distributed: Optional[Decimal] = Field(
        default=None, description="Cumulative rewards distributed", ge=0
    )
    total_supply: Optional[Decimal] = Field(
        default=None, description="Total token supply"
    )
...
class PeriodMetrics(BaseTokenomicsModel):
    ...
    minted_amount: Decimal = Field(
        default=0, ge=0, description="Amount minted in this period"
    )
    burned_amount: Decimal = Field(
        default=0, ge=0, description="Amount burned in this period"
    )
    vested_amount: Decimal = Field(
        default=0, ge=0, description="Amount vested in this period"
    )
    staked_amount: Decimal = Field(
        default=0, ge=0, description="Amount staked in this period"
    )
    staking_rewards: Decimal = Field(
        default=0, ge=0, description="Staking rewards distributed in this period"
    )
    locked_amount: Decimal = Field(
        default=0, ge=0, description="Total amount locked (staking + vesting)"
    )
...
class SimulationResponse(BaseTokenomicsModel):
    ...
    total_burned: Optional[Decimal] = Field(
        default=None, description="Total amount burned", ge=0
    )
    total_vested: Optional[Decimal] = Field(
        default=None, description="Total amount vested", ge=0
    )
    total_staked: Optional[Decimal] = Field(
        default=None, description="Final staked amount", ge=0
    )
    total_rewards: Optional[Decimal] = Field(
        default=None, description="Total rewards distributed", ge=0
    )


class ComparisonRequest(BaseTokenomicsModel):
    scenarios: List[NamedScenarioRequest] = Field(
        ..., min_length=2, max_length=5, description="List of scenarios to compare"
    )
...
class ConstantInflationRequest(BaseTokenomicsModel):
    ...
    inflation_rate: Decimal = Field(
        ..., ge=0, le=100, description="Annual inflation rate (%)"
    )
...
class BurnRequest(BaseTokenomicsModel):
    ...
    burn_rate: Optional[Decimal] = Field(
        default=None, ge=0, le=100, description="Monthly burn rate (%)"
    )
    burn_events: Optional[List[BurnEvent]] = Field(
        default=None, description="List of burn events"
    )
    ...
    def validate_burn_config(self) -> "BurnRequest":
        ...
        if self.burn_rate is not None and self.burn_events is not None:
            raise ValueError(
                "Must specify either burn_rate or burn_events, but not both"
            )
...
class ScenarioSummary(BaseTokenomicsModel):
    ...
    total_staking_rewards: Decimal = Field(
        ..., description="Total staking rewards distributed"
    )
    ...
    supply_change_percentage: Decimal = Field(
        ..., description="Percentage change in supply"
    )
...
class ScenarioResponse(BaseTokenomicsModel):
    ...
    simulation_data: Optional[List[TokenPoint]] = Field(
        None, description="Raw simulation data points"
    )
