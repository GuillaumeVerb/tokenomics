...
class ShockEvent(BaseModel):
    ...
    time_unit: TimeUnit = Field(
        default=TimeUnit.MONTH, description="Unité de temps (mois ou année)"
    )
    ...
    def validate_value(cls, v, values):
        ...
        if event_type in [EventType.MASS_BURN, EventType.INFLATION_SPIKE] and v > 1:
            raise ValueError(
                "La valeur doit être entre 0 et 1 pour les événements de burn et d'inflation"
            )
        return v
    ...
    def apply_event(self, supply: float) -> tuple[float, str]:
        ...
        if self.event_type == EventType.MASS_BURN:
            ...
            return (
                new_supply,
                f"Mass burn de {self.value*100}% des tokens en circulation",
            )
...
class SimulationEvents(BaseModel):
    events: List[ShockEvent] = Field(
        default_list=[], description="Liste des événements de shock"
    )
